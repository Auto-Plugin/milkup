...
小文件优先体验，大文件优先稳定和性能。即时渲染、复杂插件、全量大纲、实时表格布局等能力需要支持按文件规模降级。
...
| 等级 | 大小        | 模式         | 能力                             |
| ---- | ----------- | ------------ | -------------------------------- |
| S    | 0-10 MB     | 完整模式     | 完整解析、完整即时渲染、完整插件 |
| M    | 10-100 MB   | 增量模式     | 增量解析、虚拟渲染、部分后台索引 |
| L    | 100 MB-1 GB | 大文件模式   | 分块加载、局部解析、功能降级     |
| XL   | 1 GB+       | 超大文件模式 | 视口编辑、源码优先、按需渲染     |
...
- 1 MB 文档：完整即时渲染无明显延迟。
- 10 MB 文档：打开时间可接受，编辑输入稳定。
- 100 MB 文档：进入大文件模式，滚动和编辑稳定。
- 1 GB 文档：可打开、可搜索、可局部编辑、可保存。
...
- AI agent 可以读取当前文档、选区、大纲、资源、插件能力，并执行受控修改。
...
| -------- | ------------------------------------------------ |
| Tool     | Action                                           |
| Resource | 当前文档、选区、大纲、文件树、资源索引、插件列表 |
